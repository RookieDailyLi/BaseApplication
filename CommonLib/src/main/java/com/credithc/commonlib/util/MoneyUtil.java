...
public class MoneyUtil {
    ...
    public static String formatIntMoney(String srcStr) {
        String srtInt = srcStr.replaceAll(",", "");
        while (srtInt.startsWith("0") && srtInt.length() >= 2) {
            srtInt = srtInt.substring(1, srtInt.length());
        }
    ...
    public static String formatMoney(String srcStr) {
        ...
        String[] array = srtInt.split("\\.");
        if (array.length == 2) { // 小数和整数部分都存在
            String intPart = formatIntMoney(array[0]);
            stringBuilder.append(intPart);
            stringBuilder.append(".");
            if (array[1].length() > 2) { // 多于两位小数 取两位
                stringBuilder.append(array[1].substring(0, 2));
            } else {
                stringBuilder.append(array[1]); // 不多于两位拼接
            }

            return stringBuilder.toString();

        } else if (array.length == 1) {
            stringBuilder.append(formatIntMoney(array[0]));
            if (srcStr.endsWith(".")) {
                stringBuilder.append(".");
            }
            // 只有整数部分
    ...
    public static String formatIntStr(int money) {
        return formatIntMoney(String.valueOf(money));
    }
    ...
    public static String getFormatDotMoney(String money) {
        if (money == null || money.equals("0") || money.equals("0.0") || money.equals("0.") || money.equals(".")) {
            return "0.00";
        }
        BigDecimal bigDecimal = new BigDecimal(money);
        return MoneyUtil.formatMoney(bigDecimal.setScale(2, BigDecimal.ROUND_HALF_UP).toPlainString());
//        return FormatUtil.getNumberFormat(FormatUtil.str2Double(money), "#,###.##");
    }
